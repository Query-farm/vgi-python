...
def emit(out: TextIO) -> None:
    ...
    sha = vgi_python_sha()
    out.write("// ============================================================================\n")
    out.write("// GENERATED by vgi.codegen.ts_client. DO NOT EDIT BY HAND.\n")
    out.write("//\n")
    out.write(f"// Generator: vgi-gen-ts-client v{GENERATOR_VERSION}\n")
    out.write(f"// vgi-python SHA: {sha}\n")
    out.write("//\n")
    out.write("// To regenerate:\n")
    out.write("//   uv run --project ~/Development/vgi-python vgi-gen-ts-client \\\n")
    out.write("//     > ~/Development/vgi-typescript/src/generated/vgi-client.ts\n")
    out.write("// ============================================================================\n")
    out.write("\n")

    # Imports for the codec wrappers emitted at the bottom of the file.
    out.write('import { encodeASD, decodeASD } from "../codec/asd.js";\n')
    ...
    for s in info_schema_imports:
        out.write(f"  {s},\n")
    out.write('} from "./vgi-protocol-schemas.js";\n\n')
    ...
    for e in sorted(ctx.enum_order, key=lambda c: ctx.enums_seen[c]):
        out.write(_render_enum(e, ctx))
    out.write("\n")
    for cls in ctx.dataclass_order:
        out.write(_render_dataclass(cls, ctx))
        out.write("\n")
    ...
    out.write("// ----------------------------------------------------------------------------\n")
    out.write("// Codec wrappers: turn the typed interfaces above into actual wire bytes.\n")
    out.write("// ----------------------------------------------------------------------------\n\n")
