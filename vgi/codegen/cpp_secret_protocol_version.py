...
from vgi.codegen._common import DEFAULT_CPP_NAMESPACE, GeneratorError, parse_cpp_namespace
...
def current_secret_protocol_version() -> str:
    ...
    if not isinstance(value, str):
        raise TypeError(
            f"VgiSecretProtocol.protocol_version must be a str declared as a ClassVar; got {type(value).__name__}"
        )
    return value
